...
save_directory = os.path.join(args.path, "elaan", "results", "ELAANH", args.dataset, args.dist, "{}".format(args.correlation), "N_train_{}".format(args.train_size), "seed{}".format(args.seed)) 
...
if args.dataset=='synthetic':
    ...
    #     print("TRAIN:", train_index, "VAL:", val_index)
        ...
        amish = models.ELAANH(lams_sm=lams_sm,
                             lams_L0=lams_L0,
                             alpha=r,
...
#         interaction_terms = np.array(interaction_terms)
        ...
        amish.fit_validation_path(X_val, y_val)
...
#     print("TRAIN:", train_index, "VAL:", val_index)
    ...
    load_path = os.path.join(save_directory, 'ELAANI', 'v{}'.format(version), 'r{}'.format(1.0), 'fold0')
...
amish = models.ELAANH(lams_sm=lams_sm_opt,
                     lams_L0=lams_L0_opt,
                     taus=taus_opt,
                     alpha=r,
                     convergence_tolerance=convergence_tolerance,
...
amish.generate_splines_and_quadratic_penalties(Ki=Ki, Kij=Kij)
if args.dataset=='synthetic':
    amish.fit_validation_path(Xval=Xtrain, Yval=ytrain) # only matters when not running for single value.
elif args.dataset=='large-synthetic-correlated':
    amish.fit_validation_path(Xval=Xval, Yval=yval)

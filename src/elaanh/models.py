...
from src.elaani import utilities
from src.elaani import ValidationPath
from src.elaani import L0Path
from src.elaani import optimizer

from src.elaanh import ValidationPath_HS
from src.elaanh import L0Path_HS
from src.elaanh import HierarchicalSparsity
from src.elaanh import ThresholdingPath_HS
...
class ELAANH(object):
    """ELAAN-H: AM with interactions under strong hierarchy with b-splines under L0 sparsity.
    ...
    def __init__(self,
                 ...
                 lams_L0=np.logspace(start=1, stop=-3, num=25, base=10.0),
                 taus=np.logspace(start=0, stop=-2, num=50, base=10.0),
                 alpha=1.0,
    ...
    def get_main_and_interaction_set(self, active_set=None, interaction_terms=None):
        """Loads/Generates interaction effects. 
        ...
        if interaction_terms is None:
            ...
            self.interaction_terms = np.array(interaction_terms)
            self.Imax = len(self.interaction_terms)
            self.active_interaction_set = np.arange(len(self.interaction_terms))
    ...
    def fit_validation_path(self, Xval, Yval):
        self.Xval = Xval
        self.Yval = Yval
        self.beta_HS = [np.zeros((s.shape[1], 1), dtype=float) for s in self.S]
        self.delta_HS = [np.zeros((s.shape[1], 1), dtype=float) for s in self.S_interaction]

        CD_AS = (lambda Ypred, beta, zeta, active_set, lam, P, P_interaction : optimizer.CD_Joint_ActiveSet(
        ...
        MIP_HS = (lambda Ypred, beta, zeta, lam : HierarchicalSparsity.MIP_HierarchicalSparsity_ActiveSet(
            Ypred, beta, zeta, lam,
            active_set=[deepcopy(self.active_set), deepcopy(self.active_interaction_set)], Y=self.Y, B=[self.Btrain, self.Btrain_interaction], S=[self.S, self.S_interaction], I=[self.p, len(self.interaction_terms)], interaction_terms=self.interaction_terms, r=self.alpha, verbose=False, path=self.path)
        )
        Taupath = (lambda lam_1, lam_2, beta, zeta, delta, alpha, P, P_interaction : ThresholdingPath_HS.TauPath(
            lam_1, lam_2, beta, zeta, delta, alpha, P, P_interaction,
            taus=self.taus, CD_J_AS=CD_AS, active_set=deepcopy(self.active_set), active_interaction_set=deepcopy(self.active_interaction_set), B=self.Btrain, B_interaction=self.Btrain_interaction, K_main=self.K_main, K_interaction=self.K_interaction, Xval=self.Xval, Xmin=self.Xmin, Xmax=self.Xmax, Y=self.Y, Yval=self.Yval, y_scaler=self.y_scaler, S=self.S, S_interaction=self.S_interaction, interaction_terms=self.interaction_terms, eval_criteria=self.eval_criteria, path=self.path, r=self.alpha, logging=self.logging)
        )    
        L0path_HS = (lambda MIP_HS, lam_1, lams_2, beta, zeta, delta, alpha, start :L0Path_HS.L0Path(
            MIP_HS, lam_1, lams_2, beta, zeta, delta, alpha, start,
            Taupath=Taupath, active_set=deepcopy(self.active_set), active_interaction_set=deepcopy(self.active_interaction_set), 
            B=self.Btrain, B_interaction=self.Btrain_interaction, BT_B=self.BtrainT_Btrain, 
            B_interactionT_B_interaction=self.Btrain_interactionT_Btrain_interaction, Y=self.Y, S=self.S, 
            S_interaction=self.S_interaction, interaction_terms=self.interaction_terms, path=self.path, r=self.alpha, 
            logging=self.logging)
        )
        CV_HS = (lambda L0path_HS, MIP_HS: ValidationPath_HS.ValidationPath(
            L0path_HS, MIP_HS,
            beta=deepcopy(self.beta_HS), zeta=deepcopy(np.zeros((1, len(self.Btrain)), dtype=float)), 
            delta=deepcopy(self.delta_HS), alpha=deepcopy(np.zeros((1, len(self.Btrain_interaction)), dtype=float)), B=self.Btrain, B_interaction=self.Btrain_interaction, interaction_terms=self.interaction_terms, column_names=self.column_names, lams_1=self.lams_sm, lams_2=self.lams_L0, path=self.path, logging=self.logging)
        )

        start = time.time()
        self.optimal_solution, self.sparse_solution = CV_HS(L0path_HS=L0path_HS, MIP_HS=MIP_HS)
        end = time.time()
        hours, rem = divmod(end-start, 3600)
        minutes, seconds = divmod(rem, 60)
        if self.logging == True:
            with open(self.path+'/Results-HS.txt', "a") as f:
        ...
        self.beta_opt, self.delta_opt, self.zeta_opt, self.alpha_opt, self.lam_sm_opt, self.lam_L0_opt, self.tau_opt, self.J_opt, self.active_set_opt, self.active_interaction_set_opt = self.optimal_solution
        self.beta_sp, self.delta_sp, self.zeta_sp, self.alpha_sp, self.lam_sm_sp, self.lam_L0_sp, self.tau_sp, self.J_sp, self.active_set_sp, self.active_interaction_set_sp = self.sparse_solution
    ...
    def evaluate(self, X, Y, use_sparse_solution=False):
        ...
        mse, rmse = utils.metrics(Y, Ypred, y_preprocessor=self.y_scaler)
        return mse, rmse
    ...
    def evaluate_and_save(self, Xtest, Ytest):
        ...
        pen = np.array(['ELAANH-opt','ELAANH-sp'])
        M = pen.shape[0]
        ...
        train_mse_opt, train_rmse_opt = self.evaluate(self.X, self.Y, use_sparse_solution=False)
        val_mse_opt, val_rmse_opt = self.evaluate(self.Xval, self.Yval, use_sparse_solution=False)
        test_mse_opt, test_rmse_opt = self.evaluate(Xtest, Ytest, use_sparse_solution=False)
        ...
        with open(self.path+'/Results.txt', "a") as f:
            f.write('Optimal: Test-MSE: {:.6f}, Test-RMSE: {:.6f}\n'.format(test_mse_opt, test_rmse_opt)) 
        print('Optimal: Test-MSE: {:.6f}, Test-RMSE: {:.6f}'.format(test_mse_opt, test_rmse_opt))
    
        train_mse_sp, train_rmse_sp = self.evaluate(self.X, self.Y, use_sparse_solution=True)
        val_mse_sp, val_rmse_sp = self.evaluate(self.Xval, self.Yval, use_sparse_solution=True)
        test_mse_sp, test_rmse_sp = self.evaluate(Xtest, Ytest, use_sparse_solution=True)
        if self.eval_criteria == 'mse':
            train_eval_sp = train_mse_sp
            val_eval_sp = val_mse_sp
            test_eval_sp = test_mse_sp
        with open(self.path+'/Results.txt', "a") as f:
            f.write('Sparse: Test-MSE: {:.6f}, Test-RMSE: {:.6f}'.format(test_mse_sp, test_rmse_sp)) 
        print('Sparse: Test-MSE: {:.6f}, Test-RMSE: {:.6f}'.format(test_mse_sp, test_rmse_sp))    
        
        hp_opt = {'lam_sm': self.lam_sm_opt, 'lam_L0': self.lam_L0_opt}
        df.loc['ELAANH-opt', 'Training {}'.format(self.eval_criteria)] = train_eval_opt
        df.loc['ELAANH-opt', 'Validation {}'.format(self.eval_criteria)] = val_eval_opt
        df.loc['ELAANH-opt', 'Test {}'.format(self.eval_criteria)] = test_eval_opt
        df.loc['ELAANH-opt', 'Test MSE'], df.loc['ELAANH-opt', 'Test RMSE'] = (test_mse_opt, test_rmse_opt)
        df.loc['ELAANH-opt', 'Nonzeros']=len(np.union1d(self.active_set_opt, np.unique(np.array(self.interaction_terms)[self.active_interaction_set_opt])))
        df.loc['ELAANH-opt', 'Optimal Hyperparameters'] = ', '.join([f'{key}: {value}' for key, value in hp_opt.items()])
        hp_sp = {'lam_sm': self.lam_sm_sp, 'lam_L0': self.lam_L0_sp}
        df.loc['ELAANH-sp', 'Training {}'.format(self.eval_criteria)] = train_eval_sp
        df.loc['ELAANH-sp', 'Validation {}'.format(self.eval_criteria)] = val_eval_sp
        df.loc['ELAANH-sp', 'Test {}'.format(self.eval_criteria)] = test_eval_sp
        df.loc['ELAANH-sp', 'Test MSE'], df.loc['ELAANH-sp', 'Test RMSE'] = (test_mse_sp, test_rmse_sp)
        df.loc['ELAANH-sp', 'Nonzeros']=len(np.union1d(self.active_set_sp, np.unique(np.array(self.interaction_terms)[self.active_interaction_set_sp])))
        df.loc['ELAANH-sp', 'Optimal Hyperparameters'] = ', '.join([f'{key}: {value}' for key, value in hp_sp.items()])
        display(df)
        with open(os.path.join(self.path, 'ELAANH.csv'), 'a') as f:
    ...
    def interaction_effects_interpretability(self,
        ...
        """
        folderpath = os.path.join(self.path, 'InteractionInterpretability')
        os.makedirs(folderpath, exist_ok=True)
        ...
        for interaction_effect in notebook.tqdm(interaction_terms):
            ...
            plt.legend()
            plt.savefig(os.path.join(folderpath, '{}--{}.pdf'.format(self.column_names[i], self.column_names[j])), bbox_inches='tight')            
            plt.show()
            ...
            plt.legend()
            plt.savefig(os.path.join(folderpath, '{}--{}.pdf'.format(self.column_names[j], self.column_names[i])), bbox_inches='tight')            
            plt.show()                    

...
from src.elaan import ValidationPath
from src.elaan import L0Path
from src.elaan import CoordinateDescent
...
class ELAAN(object):
    """ELAAN: AMs with only main effects under L0 sparsity.
    ...
    def generate_main_terms(self, generate_all=True, Mmax=10000, subsample=0.2, Ki=4):
        ...
        if Mmax > self.M:
            ...
            self.Mmax = self.Mmax
        assert self.Mmax <= self.M, "Mmax:{} must be less than maximum number of main effects:{}".format(Mmax, self.M)
        ...
        res_p = np.argsort([item for sublist in results for item in sublist])
        if not self.generate_all:
            res_p = res_p[:self.Mmax]
        else:
            pass
        self.main_terms = np.array([self.main_terms[k] for k in res_p]) 
        print("self.main_terms:", self.main_terms)
    ...
    def fit_validation_path(self, Xval, Yval):
        ...
        VP = (lambda L0path, CD_S, CD_S_AS: ValidationPath.ValidationPath(
            L0path, CD_S, CD_S_AS,
        ...
        start = time.time()
        self.optimal_solution, self.sparse_solution, self.union_set, self.path_solution = VP(L0path=L0path, CD_S=CD_S, CD_S_AS=CD_S_AS)
...
#             train_mses_opt[i], train_rmses_opt[i] = self.evaluate(self.X[idx], self.Y[idx], use_sparse_solution=False)
...
#         standard_err = np.std(test_mses)
        ...
        df.loc['ELAAN-opt', 'Training {}'.format(self.eval_criteria)] = train_eval_opt
        df.loc['ELAAN-opt', 'Validation {}'.format(self.eval_criteria)] = val_eval_opt
        df.loc['ELAAN-opt', 'Test {}'.format(self.eval_criteria)] = test_eval_opt
        df.loc['ELAAN-opt', 'Test MSE'], df.loc['ELAAN-opt', 'Test RMSE'] = (test_mse_opt, test_rmse_opt)
        df.loc['ELAAN-opt', 'Nonzeros']=len(self.active_set_opt)
        df.loc['ELAAN-opt', 'Optimal Hyperparameters'] = ', '.join([f'{key}: {value}' for key, value in hp_opt.items()])
        hp_sp = {'lam_sm': self.lam_sm_sp, 'lam_L0': self.lam_L0_sp}
        df.loc['ELAAN-sp', 'Training {}'.format(self.eval_criteria)] = train_eval_sp
        df.loc['ELAAN-sp', 'Validation {}'.format(self.eval_criteria)] = val_eval_sp
        df.loc['ELAAN-sp', 'Test {}'.format(self.eval_criteria)] = test_eval_sp
        df.loc['ELAAN-sp', 'Test MSE'], df.loc['ELAAN-sp', 'Test RMSE'] = (test_mse_sp, test_rmse_sp)
        df.loc['ELAAN-sp', 'Nonzeros']=len(self.active_set_sp)
        df.loc['ELAAN-sp', 'Optimal Hyperparameters'] = ', '.join([f'{key}: {value}' for key, value in hp_sp.items()])
        display(df)

        with open(os.path.join(self.path, 'ELAAN.csv'), 'a') as f:

...
class ELAANI(object):
    """ELAANI: AM with interactions with b-splines under L0 sparsity.
    ...
    def fit_validation_path(self, Xval=None, Yval=None):
        # Assuming Train sets have been passed already.
        self.Xval = Xval
        self.Yval = Yval
        ...
        self.VP = (lambda L0path, CD_J, CD_J_AS: ValidationPath.ValidationPath(
            L0path, CD_J, CD_J_AS,
    ...
    def evaluate_and_save(self, Xtest, Ytest):
        ...
        pen = np.array(['ELAANI-opt','ELAANI-sp'])
        M = pen.shape[0]
        ...
        df.loc['ELAANI-opt', 'Training {}'.format(self.eval_criteria)] = train_eval_opt
        df.loc['ELAANI-opt', 'Validation {}'.format(self.eval_criteria)] = val_eval_opt
        df.loc['ELAANI-opt', 'Test {}'.format(self.eval_criteria)] = test_eval_opt
        df.loc['ELAANI-opt', 'Test MSE'], df.loc['ELAANI-opt', 'Test RMSE'] = (test_mse_opt, test_rmse_opt)
        df.loc['ELAANI-opt', 'Nonzeros']=len(np.union1d(self.active_set_opt, np.unique(np.array(self.interaction_terms)[self.active_interaction_set_opt])))
        df.loc['ELAANI-opt', 'Optimal Hyperparameters'] = ', '.join([f'{key}: {value}' for key, value in hp_opt.items()])
        hp_sp = {'lam_sm': self.lam_sm_sp, 'lam_L0': self.lam_L0_sp}
        df.loc['ELAANI-sp', 'Training {}'.format(self.eval_criteria)] = train_eval_sp
        df.loc['ELAANI-sp', 'Validation {}'.format(self.eval_criteria)] = val_eval_sp
        df.loc['ELAANI-sp', 'Test {}'.format(self.eval_criteria)] = test_eval_sp
        df.loc['ELAANI-sp', 'Test MSE'], df.loc['ELAANI-sp', 'Test RMSE'] = (test_mse_sp, test_rmse_sp)
        df.loc['ELAANI-sp', 'Nonzeros']=len(np.union1d(self.active_set_sp, np.unique(np.array(self.interaction_terms)[self.active_interaction_set_sp])))
        df.loc['ELAANI-sp', 'Optimal Hyperparameters'] = ', '.join([f'{key}: {value}' for key, value in hp_sp.items()])
        display(df)
        with open(os.path.join(self.path, 'ELAANI.csv'), 'a') as f:
